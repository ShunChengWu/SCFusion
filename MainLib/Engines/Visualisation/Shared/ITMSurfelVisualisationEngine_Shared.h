                                         ...
                                         float trackingSurfelMaxDepth, float trackingSurfelMinConfidence, Vector4f *pointsMap, Vector4f *normalsMap)
{
  ...
  if(surfelIndex >= 0)
  {
    ...
    // If the surfel is sufficiently close to the camera and has a sufficiently high confidence value:
      ...
      auto transform_point = [](const Matrix4f& T, const Vector3f& p) -> Vector3f
      {
...
/**
 * \brief Writes the label color of the surfel at a particular raster position in the index image to an output image.
 *
 * \param locId             The raster position in the index image.
 * \param surfelIndexImage  The index image.
 * \param surfels           The surfels in the scene.
 ...
 */
    template <typename TSurfel>
    _CPU_AND_GPU_CODE_
    void shade_pixel_labelColour_shared(int locId, const unsigned int *surfelIndexImage, const TSurfel *surfels, const Vector4f *labelColorList, unsigned int labelColorListSize, Vector4u *outputImage)
    {
      Vector4u col4(0, 255, 255, 255);
      ...
      if(surfelIndex >= 0)
      {
        ushort label = SurfelLabelManipulator<TSurfel::hasLabelInformation>::read(surfels[surfelIndex]);
        if(label < labelColorListSize)
          col4 = (labelColorList[label]*255).toUChar();
      }
      outputImage[locId] = col4;
    }

    /**
 * \brief Writes the label of the surfel at a particular raster position in the index image to an output image.
 *
 * \param locId             The raster position in the index image.
 * \param surfelIndexImage  The index image.
 * \param surfels           The surfels in the scene.
 ...
 */
    template <typename TSurfel>
    _CPU_AND_GPU_CODE_
    void shade_pixel_label_shared(int locId, const unsigned int *surfelIndexImage, const TSurfel *surfels, ushort *outputImage)
    {
      ushort label = 0;
      ...
      if(surfelIndex >= 0)
      {
        label = SurfelLabelManipulator<TSurfel::hasLabelInformation>::read(surfels[surfelIndex]);
      }
      outputImage[locId] = label;
    }
