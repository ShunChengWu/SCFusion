...
/** Check whether the block is visible from the given view point.
 */
__global__ void ITMLib::buildCompleteVisibleList_device(const ITMHashEntry *hashTable, ITMHashSwapState *swapStates, bool useSwapping, int noTotalEntries,
...
//    unsigned int hashIdx;
	...
	if (hashEntry.ptr >= -1) // which means have been allocated.
	{
		shouldPrefix = true;

		bool isVisible, isVisibleEnlarged;
        if(useSwapping) {
            checkBlockVisibility<true>(isVisible, isVisibleEnlarged, hashEntry.pos, M, projParams, voxelSize, imgSize);
            if (isVisibleEnlarged) hashVisibleType = 1;
        } else {
            checkBlockVisibility<false>(isVisible, isVisibleEnlarged, hashEntry.pos, M, projParams, voxelSize, imgSize);
            if (isVisible) hashVisibleType = 1;
        }
