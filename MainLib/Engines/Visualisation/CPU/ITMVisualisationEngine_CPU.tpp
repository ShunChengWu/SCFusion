	...
	ITMRenderState *renderState) const
{
	...
	//build visible list
	for (int targetIdx = 0; targetIdx < noTotalEntries; targetIdx++)/// 傳遞過所有的entries看有哪個voxel block在視角內
	{
...
template<class TVoxel, class TIndex>
void ITMVisualisationEngine_CPU<TVoxel, TIndex>::CreateExpectedDepths(const ITMScene<TVoxel,TIndex> *scene, const ORUtils::SE3Pose *pose, const ITMIntrinsics *intrinsics, const Vector2f &viewFrustumRange, ITMRenderState *renderState) const
{
	Vector2i imgSize = renderState->renderingRangeImage->noDims;
...
//		pixel.x = 0.2f;
//		pixel.y = 3.0f;
        pixel.x = viewFrustumRange.x; pixel.y = viewFrustumRange.y;
	}
}

template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHash>::CreateExpectedDepths(const ITMScene<TVoxel,ITMVoxelBlockHash> *scene, const ORUtils::SE3Pose *pose, const ITMIntrinsics *intrinsics, const Vector2f &viewFrustumRange,
	ITMRenderState *renderState) const
{
	Vector2i imgSize = renderState->renderingRangeImage->noDims;
	Vector2f *minmaxData = renderState->renderingRangeImage->GetData(MEMORYDEVICE_CPU);
...
static void GenericRaycast(const ITMScene<TVoxel, TIndex> *scene, const Vector2i& imgSize, const Matrix4f& invM,
        const Vector4f& projParams, const ITMRenderState *renderState, ITMLib::IITMVisualisationEngine::RenderMode renderMode, bool updateVisibleList)
{
...
static void RenderImage_common(const ITMScene<TVoxel,TIndex> *scene, const ORUtils::SE3Pose *pose, const ITMIntrinsics *intrinsics,
	...
	IITMVisualisationEngine::RenderRaycastSelection raycastType, ITMLib::IITMVisualisationEngine::RenderMode renderMode)
{
	...
	case IITMVisualisationEngine::RENDER_COLOUR_FROM_CONFIDENCE:
		...
		break;
    case IITMVisualisationEngine::RENDER_LABELCOLOUR_FROM_IMAGENORMALS://TODO: implement this
    case IITMVisualisationEngine::RENDER_LABELCOLOUR_FROM_VOLUME:
        ...
        for (int locId = 0; locId < imgSize.x * imgSize.y; locId++)
        {
            //processPixelLabel_ImageNormals(outRendering,pointsRay,labelsRay,)
//            Vector4f ptRay = pointsRay[locId];
        }
        break;
...
static void CreatePointCloud_common(const ITMScene<TVoxel,TIndex> *scene, const ITMView *view, ITMTrackingState *trackingState, 
	ITMRenderState *renderState, ITMLib::IITMVisualisationEngine::RenderMode renderMode, bool skipPoints)
{
	...
	GenericRaycast(scene, imgSize, invM, view->calib.intrinsics_rgb.projectionParamsSimple.all, renderState, renderMode, true);
        ...
        ITMRenderState *renderState, ITMLib::IITMVisualisationEngine::RenderMode renderMode)
{
	...
	// the list of visible blocks if possible
	GenericRaycast(scene, imgSize, invM, view->calib.intrinsics_d.projectionParamsSimple.all, renderState, renderMode, true);
	trackingState->pose_pointCloud->SetFrom(trackingState->pose_d);
	...
	for (int y = 0; y < imgSize.y; y++) for (int x = 0; x < imgSize.x; x++)
	{
		...
		{
#ifndef NDEBUG
            counter += processPixelICP<true, true>(pointsMap, normalsMap, pointsRay, imgSize, x, y, voxelSize, lightSource);
#else
		...
		{
#ifndef NDEBUG
            counter += processPixelICP<true, false>(pointsMap, normalsMap, pointsRay, imgSize, x, y, voxelSize, lightSource);
#else
...
#ifndef NDEBUG
printf("[ITMVisualizationEngine_CPU.tpp][%s]counter: %d\n", __FUNCTION__, counter);
#endif
}
        ...
        ITMRenderState *renderState, ITMLib::IITMVisualisationEngine::RenderMode renderMode)
{
	...
	const auto &invProjParams = InvertProjectionParams(projParams);
	...
	for (int pointId = 0; pointId < noMissingPoints; pointId++)
	{
		...
		castRay<TVoxel, TIndex, false>(forwardProjection[locId], NULL, x, y, voxelData, voxelIndex, invM, invProjParams,
			1.0f / scene->sceneParams->voxelSize, scene->sceneParams->mu, minmaximg[locId2], renderMode == ITMLib::IITMVisualisationEngine::RenderMode::RENDER_PERSPECTIVE);
	}
}
...
{
	RenderImage_common(scene, pose, intrinsics, renderState, outputImage, type, raycastType, renderMode);
}
...
template<class TVoxel>
void ITMVisualisationEngine_CPU<TVoxel,ITMVoxelBlockHash>::GetVertexImage(const ITMScene<TVoxel,ITMVoxelBlockHash> *scene, const ORUtils::SE3Pose *pose, const ITMIntrinsics *intrinsics,
    ...
    auto inputData = renderState->raycastResult->GetData(MEMORYDEVICE_CPU);
    auto outputData = outputImage->GetData(MEMORYDEVICE_CPU);
    for(uint i=0;i<outputImage->dataSize;++i) {
        outputData[i] = inputData[i].toVector3() * scene->sceneParams->voxelSize;
    }
}


template<class TVoxel, class TIndex>
void ITMVisualisationEngine_CPU<TVoxel, TIndex>::FindSurface(const ITMScene<TVoxel,TIndex> *scene, const ORUtils::SE3Pose *pose,
        const ITMIntrinsics *intrinsics, const ITMRenderState *renderState, ITMLib::IITMVisualisationEngine::RenderMode renderMode) const
{
	// this one is generally done for freeview visualisation, so no, do not
	// update the list of visible blocks
	GenericRaycast(scene, renderState->raycastResult->noDims, pose->GetInvM(), intrinsics->projectionParamsSimple.all, renderState,renderMode, false);
}
...
{ 
	CreatePointCloud_common(scene, view, trackingState, renderState, renderMode, skipPoints);
}
...
{
	CreateICPMaps_common(scene, view, trackingState, renderState, renderMode);
}
...
void ITMVisualisationEngine_CPU<TVoxel, TIndex>::ForwardRender(const ITMScene<TVoxel,TIndex> *scene, const ITMView *view, ITMTrackingState *trackingState, 
	ITMRenderState *renderState, ITMLib::IITMVisualisationEngine::RenderMode renderMode) const
{
	ForwardRender_common(scene, view, trackingState, renderState, renderMode);
}
