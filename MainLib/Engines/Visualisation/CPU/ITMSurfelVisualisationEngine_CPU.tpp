                                                            ...
                                                            ITMUChar4Image *outputImage, RenderImageType type) const
{
  ...
  const unsigned int *surfelIndexImagePtr = renderState->GetIndexImage()->GetData(MEMORYDEVICE_CPU);
  const TSurfel *surfels = scene->GetSurfels()->GetData(MEMORYDEVICE_CPU);
  const Vector4f *labelColorList = Base::labelColorList_? Base::labelColorList_->GetDataConst(MEMORYDEVICE_CPU) : nullptr;
  unsigned int labelColorListSize = Base::labelColorList_ ? Base::labelColorList_->dataSize : 0;
...
#pragma omp parallel for
          ...
          for(int locId = 0; locId < pixelCount; ++locId)
          {
              shade_pixel_labelColour_shared(locId, surfelIndexImagePtr, surfels, labelColorList, labelColorListSize, outputImagePtr);
          }
      }
    default:
    {
                                                               ...
                                                               UnstableSurfelRenderingMode unstableSurfelRenderingMode, int *depthBuffer) const
{
  ...
  for(int surfelId = 0; surfelId < surfelCount; ++surfelId)
  {
    ...
    template<typename TSurfel>
    void ITMSurfelVisualisationEngine_CPU<TSurfel>::RenderLabel(const ITMSurfelScene<TSurfel> *scene,
                                                                const ORUtils::SE3Pose *pose,
                                                                const ITMSurfelRenderState *renderState,
                                                                ITMUShortImage *outputImage) const {
        auto *outputImagePtr = outputImage->GetData(MEMORYDEVICE_CPU);
        const int pixelCount = static_cast<int>(outputImage->dataSize);
        const unsigned int *surfelIndexImagePtr = renderState->GetIndexImage()->GetData(MEMORYDEVICE_CPU);
        const TSurfel *surfels = scene->GetSurfels()->GetData(MEMORYDEVICE_CPU);
...
#pragma omp parallel for
        ...
        for(int locId = 0; locId < pixelCount; ++locId)
        {
            shade_pixel_label_shared(locId, surfelIndexImagePtr, surfels, outputImagePtr);
        }
    }
