...
int ITMSwappingEngine_CPU<TVoxel, ITMVoxelBlockHash>::LoadFromGlobalMemory(ITMScene<TVoxel, ITMVoxelBlockHash> *scene)
{
	...
	int *neededEntryIDs_global = globalCache->GetNeededEntryIDs(false); // 其實與neededEntryIDs_local是一樣的
	...
	for (int entryId = 0; entryId < noTotalEntries; entryId++)
	{
		...
		if (swapStates[entryId].state == 1) // data both on the host and in active memory.
		{
			neededEntryIDs_local[noNeededEntries] = entryId; // 設定 globalCache->GetNeededEntryIDs
			noNeededEntries++;
		}
	}
	...
	if (noNeededEntries > 0)
	{
		...
		{
			int entryId = neededEntryIDs_global[i]; // 讀取 剛剛設定的 globalCache->GetNeededEntryIDs

			if (globalCache->HasStoredData(entryId)) // 重點在這，查看是否有儲存的資料需要被轉移到global
			{
...
void ITMSwappingEngine_CPU<TVoxel, ITMVoxelBlockHash>::SaveToGlobalMemory(ITMScene<TVoxel, ITMVoxelBlockHash> *scene, ITMRenderState *renderState)
{
	...
	for (int entryDestId = 0; entryDestId < noTotalEntries; entryDestId++)
	{
		...
		if (swapState.state == 2 && localPtr >= 0 && entriesVisibleType[entryDestId] == 0)
		{
			...
			swapStates[entryDestId].state = 0; // 改為只在host，不在active memory
