...
//	sigma_z = z*z*1e-1;
	...
	for (int i = -2, count = 0; i <= 2; i++) for (int j = -2; j <= 2; j++, count++)
	{
		...
		w = exp(-((abs(i) + abs(j))*MEAN_SIGMA_L*MEAN_SIGMA_L + dz * sigma_z));
		w_sum += w;
		final_depth += w*tmpz;
	}
...
_CPU_AND_GPU_CODE_ inline void computeVertex(const CONSTPTR(float) *depth_in, DEVICEPTR(Vector3f) *vertex_out, int x, int y, Vector2i imgDims, Vector4f intrinparam)
{
    int idx = x + y * imgDims.x;
    
    float z = depth_in[idx];
    if (z < 0.0f)
    {
        vertex_out[idx].z = -1.0f;
        return;
    }
    
    // get z value
    vertex_out[idx].z = depth_in[idx];
    
    
    if (vertex_out[idx].z < 0)
    {
        vertex_out[idx].z = -1.0f;
        return;
    }
    ...
    // unprojected
    vertex_out[idx].x = vertex_out[idx].z * (x - intrinparam.z) / intrinparam.x;
    vertex_out[idx].y = vertex_out[idx].z * (y - intrinparam.w) / intrinparam.y;
}
...
_CPU_AND_GPU_CODE_ inline void computeNormalAndWeight(const CONSTPTR(Vector3f) *vertex_in, DEVICEPTR(Vector4f) *normal_out, DEVICEPTR(float) *sigmaZ_out, int x, int y, Vector2i imgDims, Vector4f intrinparam)
{
	Vector3f outNormal;

	int idx = x + y * imgDims.x;

	float z = vertex_in[idx].z;
	if (z < 0.0f)
	{
    ...
    xp1_y = vertex_in[(x+1) + y*imgDims.x];
    xm1_y = vertex_in[(x-1) + y*imgDims.x];
    x_yp1 = vertex_in[(x) + (y+1)*imgDims.x];
    x_ym1 = vertex_in[(x) + (y-1)*imgDims.x];
	...
	// now compute weight
	float theta = SafeAcos(outNormal.z);
