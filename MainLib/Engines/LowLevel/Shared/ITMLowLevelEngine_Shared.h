...
_CPU_AND_GPU_CODE_ inline void addBilateralFilterElem(float depth, unsigned short coef, float refDepth, float depthPrecision, float& total, float& totalCoef){
    if (depth > 0){
        float diff = refDepth - depth;
        //float bilateralCoef = exp(-diff*diff / (2 * depthVariance) ) ;
        //float bilateralCoef = exp(-diff*diff * depthPrecision);
        float bilateralCoef = exp(-diff*diff * depthPrecision);
        //float bilateralCoef = -diff*diff * depthPrecision ;
...
_CPU_AND_GPU_CODE_ inline float bilateral5x5_shared(int x, int y, int width, const float *data, float depthPrecision){
    float refDepth = data[y*width+x];
    if(refDepth <= 0) return 0;
    float smoothDepth = 0;
    float totalCoef = 0;
    addBilateralFilterElem(data[(y-2)*width+x-2], 1, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-2)*width+x-1], 4, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-2)*width+x], 7, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-2)*width+x+1], 4, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-2)*width+x+2] , 1, refDepth, depthPrecision, smoothDepth, totalCoef);

    addBilateralFilterElem(data[(y-1)*width+x-2], 4,  refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-1)*width+x-1], 16, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-1)*width+x], 26, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-1)*width+x+1], 16, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y-1)*width+x+2], 4,  refDepth, depthPrecision, smoothDepth, totalCoef);

    addBilateralFilterElem(data[(y)*width+x-2], 7,  refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y)*width+x-1], 26, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(refDepth,                    41, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y)*width+x+1], 26, refDepth, depthPrecision, smoothDepth, totalCoef);
    addBilateralFilterElem(data[(y)*width+x+2], 7,  refDepth, depthPrecision, smoothDepth, totalCoef);
