...
void ITMLowLevelEngine_CPU::BilateralFilter5x5(ITMFloatImage *img, float depthUncertaintyCoef) const
{
    img->UpdateDeviceFromHost();
    ITMFloatImage tmp(*img);
    const float *data =tmp.GetDataConst(MEMORYDEVICE_CPU);
    float *filtered_data = img->GetData(MEMORYDEVICE_CPU);
    int width = img->noDims.x;

    for(int y=0; y < img->noDims.y; ++y){
        for(int x=0; x < img->noDims.x; ++x){
            float refDepth = data[y*width+x];
            float depthSigma = depthUncertaintyCoef * refDepth * refDepth * 1e-1f;
