...
_CPU_AND_GPU_CODE_
inline TSurfel make_surfel(int locId, const Matrix4f& T, const Vector4f *vertexMap, const Vector3f *normalMap, const float *radiusMap,
                           ...
                           int timestamp)
{
  ...
  ushort label = labelMap?  labelMap[locId] : 0;
  SurfelLabelManipulator<TSurfel::hasLabelInformation>::write(surfel, label);
  if(label > 0) SurfelLabelManipulator<TSurfel::hasLabelInformation>::write_conf(surfel, 1);
  else SurfelLabelManipulator<TSurfel::hasLabelInformation>::write_conf(surfel, 0);
  return surfel;
}
...
inline TSurfel merge_surfels(const TSurfel& target, const TSurfel& source, float maxSurfelRadius, bool shouldMergeProperties, RadiusCombinationMode radiusCombinationMode)
{
  ...
  if(shouldMergeProperties)
  {
    ...
    // Compute new label and its weight
    ushort resultLabel = SurfelLabelManipulator<TSurfel::hasLabelInformation>::read(result);
    float resultWLabel = SurfelLabelManipulator<TSurfel::hasLabelInformation>::read_conf(result);
    ushort sourceLabel = SurfelLabelManipulator<TSurfel::hasLabelInformation>::read(source);
    float sourceWLabel = SurfelLabelManipulator<TSurfel::hasLabelInformation>::read_conf(source);
    ushort label;
    float w_label;
    const float maxW = 50;
    if (resultLabel == sourceLabel) {
      label = resultLabel;
      w_label = resultWLabel + sourceWLabel;
    } else {
      if(resultWLabel > sourceWLabel){
         label = resultLabel;
         w_label = resultWLabel - sourceWLabel;
      } else if (resultWLabel < sourceWLabel) {
        label = sourceLabel;
        w_label = sourceWLabel - resultWLabel;
      } else {
          label = sourceLabel;
          w_label = 1;
      }
    }
    if(w_label>maxW) w_label=maxW;

    SurfelLabelManipulator<TSurfel::hasLabelInformation>::write(result, label);
    SurfelLabelManipulator<TSurfel::hasLabelInformation>::write_conf(result, w_label);
  }
                           ...
                           TSurfel *newSurfels)
{
  ...
  if(newPointsMask[locId])
  {
    ...
    TSurfel surfel = make_surfel<TSurfel>(
      locId, T, vertexMap, normalMap, radiusMap, colourMap, labelMap, depthMapWidth, depthMapHeight, colourMapWidth, colourMapHeight,
                                      ...
                                      int supersamplingFactor, const TSurfel *surfels, unsigned int *correspondenceMap, unsigned short *newPointsMask)
{
  ...
  newPointsMask[locId] = bestSurfelIndex >= 0 ? 0 : 1;
    ...
    _CPU_AND_GPU_CODE_ inline double SafeAcos (double x)
    {
        if (x < -1.0) x = -1.0 ;
        else if (x > 1.0) x = 1.0 ;
        return acos (x) ;
                                  ...
                                  float stableSurfelConfidence, float maxMergeDist, float maxMergeAngle, float minRadiusOverlapFactor, unsigned int *mergeTargetMap)
{
  ...
  for(int i = 0; i < 4; ++i)
  {
    ...
    float angle = SafeAcos(dot(surfel.normal, neighbourSurfel.normal));
