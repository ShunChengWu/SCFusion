...
ITMSceneReconstructionEngine_CPU<TVoxel,ITMVoxelBlockHash>::ITMSceneReconstructionEngine_CPU()
{
...
void ITMSceneReconstructionEngine_CPU<TVoxel,ITMVoxelBlockHash>::ResetScene(ITMScene<TVoxel, ITMVoxelBlockHash> *scene)
{
    ...
    // Reset voxels, reset the allocation list and reset the lastFreeBlockId
	...
	tmpEntry.ptr = -2; // which means unallocated ( if <-1 )
                                                                                     ...
                                                                                     const Matrix4f &M_d, const ITMRenderState *renderState)
{
    ...
    if(TVoxel::hasLabelInformation)
        label = view->label->GetData(MEMORYDEVICE_CPU);
        ...
        for (int z = 0; z < SDF_BLOCK_SIZE; z++) for (int y = 0; y < SDF_BLOCK_SIZE; y++) for (int x = 0; x < SDF_BLOCK_SIZE; x++)
                {
                    ...
                    ComputeUpdatedVoxelInfo<TVoxel::hasColorInformation,TVoxel::hasConfidenceInformation,
                    TVoxel::hasLabelInformation, TVoxel>::compute(localVoxelBlock[locId], pt_model, M_d,
                            projParams_d, M_rgb, projParams_rgb, voxelSize, mu, maxW, depth, label, confidence, depthImgSize, rgb, rgbImgSize, scene->sceneParams->integratePolicy);
                }
    }
}
template<class TVoxel>
void ITMSceneReconstructionEngine_CPU<TVoxel, ITMVoxelBlockHash>::IntegrateIntoScene(ITMScene<TVoxel, ITMVoxelBlockHash> *scene, const ITMRenderState *renderState,
    ...
    AllocationSceneFromPoints(scene, points, renderState);

    /// Integrate
    auto label_data = labels? labels->GetData(MEMORYDEVICE_CPU) : nullptr;
    auto instance_data = instances? instances->GetData(MEMORYDEVICE_CPU) : nullptr;
    float oneOverVoxelSize = 1.f / scene->sceneParams->voxelSize;
    for(size_t i=0;i<points->dataSize;++i)
        integrateIntoScene_shared<TVoxel, ITMVoxelBlockHash>(i, scene->localVBA->GetVoxelBlocks(), scene->index->getIndexData(), oneOverVoxelSize,
...
#pragma omp parallel for
    ...
    for (int locId = 0; locId < depthImgSize.x*depthImgSize.y; locId++)
    {
        ...
        buildHashAllocAndVisibleTypePP(entriesAllocType, entriesVisibleType, x, y, blockCoords, depth, invM_d,
                                       ...
                                       scene->sceneParams->viewFrustum_max, scene->sceneParams->useInverseSensorModel);
    }
    ...
    for (int targetIdx = 0; targetIdx < noTotalEntries; targetIdx++)
    {
        ...
        if (hashVisibleType == 3)// Visible from previous frame and unstreamed -> check visibility
        {
        ...
        if (useSwapping)
        {
            if (hashVisibleType > 0 && swapStates[targetIdx].state != 2) swapStates[targetIdx].state = 1;// The target can be seen and not awaiting to be processed: Set to 1
        }
	...
	const ITMTrackingState *trackingState, const ITMRenderState *renderState)
{
	...
	float *depth = view->depth->GetData(MEMORYDEVICE_CPU);
    ushort *label = view->label->GetData(MEMORYDEVICE_CPU);
	Vector4u *rgb = view->rgb->GetData(MEMORYDEVICE_CPU);
	...
	for (int locId = 0; locId < scene->index->getVolumeSize().x*scene->index->getVolumeSize().y*scene->index->getVolumeSize().z; ++locId)
	{
		...
		ComputeUpdatedVoxelInfo<TVoxel::hasColorInformation, TVoxel::hasConfidenceInformation, TVoxel::hasLabelInformation, TVoxel>::compute(voxelArray[locId], pt_model, M_d, projParams_d, M_rgb, projParams_rgb, voxelSize, mu, maxW,
			depth, label, depthImgSize, rgb, rgbImgSize, scene->sceneParams->integratePolicy);
	}
}
