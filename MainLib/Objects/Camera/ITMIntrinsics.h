...
namespace ITMLib
{
	...
	public:
        ...
        ITMIntrinsics MakeRescaled(float scale) const
        {
            float fxNew = projectionParamsSimple.fx * scale;
            float fyNew = projectionParamsSimple.fy * scale;
            float pxNew = projectionParamsSimple.px * scale;
            float pyNew = projectionParamsSimple.py * scale;

            ITMIntrinsics intrinsics;
            intrinsics.SetFrom(imgSize.width*scale, imgSize.height*scale, fxNew, fyNew, pxNew, pyNew);
            return intrinsics;
        }
