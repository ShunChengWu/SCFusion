...
bool ITMLib::readDistortionCalib(std::istream &src, ITMLib::ITMDistortionCalib &dest)
{
	std::vector<float> tmp(8,0);
...
//    size_t counter = 0;
    ...
    for (size_t i=0; i < tmp.size(); ++i)
    {
        ...
        float tmpFloat=0;
        try {
            tmpFloat = std::stof(tmpString);
        } catch(...) {
            // If cannot convert just ignore.
        }
        
        tmp[i] = std::stof(tmpString);
    }
...
bool ITMLib::readDistortionCalib(const char *fileName, ITMDistortionCalib &dest)
{
    std::ifstream f(fileName);
    return readDistortionCalib(f, dest);
}

bool ITMLib::readRGBDCalib(std::istream & src, ITMRGBDCalib & dest)
{
	if (!ITMLib::readIntrinsics(src, dest.intrinsics_rgb)) return false;
    if (!ITMLib::readDistortionCalib(src, dest.distortionCalib_rgb)) return false;
	if (!ITMLib::readIntrinsics(src, dest.intrinsics_d)) return false;
    if (!ITMLib::readDistortionCalib(src, dest.distortionCalib_d)) return false;
...
bool ITMLib::readRGBDCalib(const char *rgbIntrinsicsFile, const char *depthIntrinsicsFile, const char *disparityCalibFile, const char *extrinsicsFile, const char * distortionCalibFile, ITMRGBDCalib & dest)
{
	bool ret = true;
	ret &= ITMLib::readIntrinsics(rgbIntrinsicsFile, dest.intrinsics_rgb);
    ret &= ITMLib::readDistortionCalib(distortionCalibFile, dest.distortionCalib_rgb);
	ret &= ITMLib::readIntrinsics(depthIntrinsicsFile, dest.intrinsics_d);
    ret &= ITMLib::readDistortionCalib(distortionCalibFile, dest.distortionCalib_d);
