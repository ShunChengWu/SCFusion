...
namespace ITMLib {
	...
	public:
        ...
        static const CONSTPTR(bool) hasLabelInformation = TVoxel::hasLabelInformation;
	};
}
...
template<class TMultiVoxel, class TMultiIndex>
_CPU_AND_GPU_CODE_ inline float readFromSDF_float_uninterpolated(const TMultiVoxel *voxelData, const TMultiIndex *voxelIndex, const Vector3f & point, int & vmIndex, ushort &label, ITMLib::ITMMultiCache & _cache)
{
    typedef typename TMultiVoxel::VoxelType TVoxel;
    typedef typename TMultiIndex::IndexType TIndex;
    accessLabel<TVoxel, TVoxel::hasLabelInformation> aclabel;
    ushort max_label=0;
    int max_wLabel=0;
    float sum_sdf = 0.0f;
    int sum_weights = 0;
    ...
    for (int localMapId = 0; localMapId < voxelIndex->numLocalMaps; ++localMapId)
    {
        ...
        if(aclabel.getWeight(v) > max_wLabel) {
            max_wLabel = aclabel.getWeight(v);
            max_label = aclabel.getLabel(v);
        }
    }
    label = max_label;
    if (sum_weights == 0) return 1.0f;
...
template<class TMultiVoxel, class TMultiIndex>
_CPU_AND_GPU_CODE_ inline float readFromSDF_float_interpolated(const TMultiVoxel *voxelData, const TMultiIndex *voxelIndex, const Vector3f & point, int & vmIndex, ushort &label, ITMLib::ITMMultiCache & _cache)
{
    typedef typename TMultiIndex::IndexType TIndex;
    ...
    for (int localMapId = 0; localMapId < voxelIndex->numLocalMaps; ++localMapId)
    {
        ...
        float sdf = readFromSDF_float_interpolated(voxelData->voxels[localMapId], voxelIndex->index[localMapId], point_local, vmIndex_tmp, cache, label, maxW);
        if (!vmIndex_tmp) continue;
...
template<class TMultiVoxel, class TMultiIndex>
_CPU_AND_GPU_CODE_ inline float readWithConfidenceFromSDF_float_interpolated(THREADPTR(float) &confidence, const CONSTPTR(TMultiVoxel) *voxelData,
                                                                             const CONSTPTR(TMultiIndex) *voxelIndex, Vector3f point, THREADPTR(int) &vmIndex, ushort &label, ITMLib::ITMMultiCache & _cache)
{
    typedef typename TMultiIndex::IndexType TIndex;
