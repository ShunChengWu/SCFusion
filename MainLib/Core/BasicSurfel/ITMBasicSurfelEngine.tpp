...
ITMBasicSurfelEngine<TSurfel>::ITMBasicSurfelEngine(const ITMLibSettings *settings, const ITMRGBDCalib* calib)
{
	this->settings = settings;
    auto imgSize_d = calib->intrinsics_d.imgSize;
    auto imgSize_rgb = calib->intrinsics_rgb.imgSize;

    LabelColorList_.reset(new ORUtils::MemoryBlock<Vector4f>(std::numeric_limits<ushort>::max(), true,true));
    ORUtils::LabelColorGenerator::Run(LabelColorList_.get(), LabelColorList_->dataSize, true, true, settings->labelColorPath, true);
	...
	surfelVisualisationEngine->SetLabelColorListPtr(LabelColorList_.get());
...
template <typename TSurfel>
void ITMBasicSurfelEngine<TSurfel>::SaveToFile(const std::string &pth_to_folder)
{
	// Not yet implemented for surfel scenes
}
...
template <typename TSurfel>
ITMTrackingState::TrackingResult ITMBasicSurfelEngine<TSurfel>::ProcessFrame(ITMFloatImage *imgDepth, ITMUChar4Image *imgColor, size_t img_counter,
                                     ORUtils::Matrix4<float> *customPose, ITMLib::ITMIMUMeasurement *imuMeasurement, ITMUShortImage *imgLabel)
{
	// prepare image and turn it into a depth image
	if (imuMeasurement == nullptr) viewBuilder->UpdateView(&view, imgColor, imgDepth, imgLabel, settings->useBilateralFilter, ITMSurfelT::hasLabelInformation);
	else viewBuilder->UpdateView(&view, imgColor, imgDepth, settings->useBilateralFilter, imuMeasurement);
    ...
    if (gtPoseMode != GTPOSEMODE_IGNORE) {
        trackingState->pose_d->SetM(customPose->inv());
    ...
    if(gtPoseMode != GTPoseMODE::GTPOSEMODE_TACKOVER)
    {
        ...
        if(std::isnan(trackingState->pose_d->GetM().m[0])){
        ...
        switch (settings->behaviourOnFailure){
            ...
            case ITMLib::ITMLibSettings::FAILUREMODE_STOP_INTEGRATION:
                if(trackingState->trackerResult != ITMLib::ITMTrackingState::TRACKING_FAILED)
                    trackerResult = trackingState->trackerResult;
	...
	if (settings->behaviourOnFailure == ITMLibSettings::FAILUREMODE_RELOCALISE)
	{
		...
		bool hasAddedKeyframe = relocaliser->ProcessFrame(view->depth.get(), trackingState->pose_d, 0, 1, &NN, &distances, trackerResult == ITMTrackingState::TRACKING_GOOD && relocalisationCount == 0);
...
template <typename TSurfel>
Vector2i ITMBasicSurfelEngine<TSurfel>::GetImageSize() const
{
...
ITMBasicSurfelEngine<TSurfel>::ToSurfelImageType(GetImageType getImageType)
{
	switch (getImageType)
	{
	    ...
	    case InfiniTAM_IMAGE_FREECAMERA_LABEL_FROM_IMAGENORMAL:
	        return ITMSurfelVisualisationEngine<TSurfel>::RENDER_LABELCOLOUR;
		default:
...
template <typename TSurfel>
void ITMBasicSurfelEngine<TSurfel>::renderImage(ITMUChar4Image *out, GetImageType getImageType, ORUtils::SE3Pose *pose, ITMIntrinsics *intrinsics,
	...
	switch (getImageType) {
        case InfiniTAM_IMAGE_ORIGINAL_RGB:
            ...
            if (settings->deviceType == ITMLibSettings::DEVICE_CUDA)
                out->SetFrom(view->rgb.get(), ORUtils::MemoryBlock<Vector4u>::CUDA_TO_CPU);
            else out->SetFrom(view->rgb.get(), ORUtils::MemoryBlock<Vector4u>::CPU_TO_CPU);
            break;
        case InfiniTAM_IMAGE_ORIGINAL_DEPTH:
            out->ChangeDims(view->depth->noDims);
            if (settings->deviceType == ITMLibSettings::DEVICE_CUDA) view->depth->UpdateHostFromDevice();
            IITMVisualisationEngine::DepthToUchar4(out, view->depth.get());
            break;
        ...
        case InfiniTAM_IMAGE_COLOUR_FROM_CONFIDENCE: {
            ...
            auto surfelImageType = ToSurfelImageType(getImageType);
