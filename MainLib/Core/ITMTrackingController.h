	...
	public:
			...
			const ITMVisualisationEngine<TVoxel,TIndex> *visualisationEngine, ITMRenderState *renderState)
		{
			...
			if (requiresColourRendering)
			{
				...
				visualisationEngine->CreateExpectedDepths(scene, &pose_rgb, &(view->calib.intrinsics_rgb), Vector2f(scene->sceneParams->viewFrustum_min, scene->sceneParams->viewFrustum_max), renderState);
				visualisationEngine->CreatePointCloud(scene, view, trackingState, renderState,
				        ITMLib::IITMVisualisationEngine::RenderMode::RENDER_PERSPECTIVE, settings->skipPoints);
				trackingState->age_pointCloud = 0;
			}
			else
			{
				visualisationEngine->CreateExpectedDepths(scene, trackingState->pose_d, &(view->calib.intrinsics_d), Vector2f(scene->sceneParams->viewFrustum_min, scene->sceneParams->viewFrustum_max), renderState);
				if (requiresFullRendering)
				{
