	...
	*/
	...
	public:
		...
		*/
		...
		/** Inverse Sensor Model allocates blocks from [minimum view frustum, depth measurement + mu]. Otherwise,
		 * allocates from [depth measurement - mu, depth measuremnt + mu] */
        bool useInverseSensorModel;

		SCFUSION::Policy::Integrate integratePolicy;

		ITMSceneParams() {}

		ITMSceneParams(float mu, int maxW, float voxelSize,
                       float viewFrustum_min, float viewFrustum_max, bool useInverseSensorModel, bool stopIntegratingAtMaxW, SCFUSION::Policy::Integrate integratePolicy)
		{
			this->mu = mu;
			this->maxW = maxW;
			this->voxelSize = voxelSize;
			this->viewFrustum_min = viewFrustum_min; this->viewFrustum_max = viewFrustum_max;
			this->useInverseSensorModel = useInverseSensorModel;
			this->stopIntegratingAtMaxW = stopIntegratingAtMaxW;
            this->integratePolicy = integratePolicy;
		}
		...
		void SetFrom(const ITMSceneParams *sceneParams)
		{
			...
			this->useInverseSensorModel = sceneParams->useInverseSensorModel;
			this->mu = sceneParams->mu;
			this->maxW = sceneParams->maxW;
			this->stopIntegratingAtMaxW = sceneParams->stopIntegratingAtMaxW;
            this->integratePolicy = sceneParams->integratePolicy;
		}
	};
}
