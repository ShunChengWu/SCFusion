...
//#include <stb_image_write.h>
    ...
    vec3uc* SensorData::RGBDFrame::decompressColorAlloc_stb(COMPRESSION_TYPE_COLOR type) const {	//can handle PNG, JPEG etc.
        ...
        int channels = 3;
        int width, height;
        unsigned char* raw = stbi_load_from_memory(m_colorCompressed, (int)m_colorSizeBytes, &width, &height, NULL, channels);
    ...
    unsigned short* SensorData::RGBDFrame::decompressDepthAlloc_stb(COMPRESSION_TYPE_DEPTH type) const {
        ...
        unsigned short* res;
        int len;
        res = (unsigned short*)stbi_zlib_decode_malloc((const char*)m_depthCompressed, (int)m_depthSizeBytes, &len);
        return res;
    }
}